...
if te.doPhotonCounting == True:
	...
	pedestalTraces = tm.extractDualSubtraces(traceList, [te.windowParametersX['pedestalLimits'], te.windowParametersX['pedestalLimits']], invert = False)
	pedestalSums = tm.sumTraces(pedestalTraces)
	...
	offsets = tm.getDualAvgMedian(pedestalSums, [te.dataParametersX['pedestalInterval'],te.dataParametersX['pedestalInterval']], invert = False)
	traceListCorrected = tm.pedestalDualSubtractions(traceList, offsets)
	del traceList

	signalTracesCor = tm.extractDualSubtraces(traceListCorrected, [te.windowParametersX['signalLimits'],te.windowParametersX['signalLimits']], invert=True)
	signalSums = tm.sumTraces(signalTracesCor)
	...
	if te.doDoubleRejection == True:
		...
		acceptedTraces, rejectedTraces, good, bad = tm.doubleRejection(CsITraces, te.windowParametersX,
	...
	if te.doPeakFinder == True:
		...
		if te.doDoubleRejection == True: 
			photonInd, countedPhotons = tm.peakFinder(WLSTraces.drop(WLSTraces[bad].columns,axis=1), te.windowParametersX, te.dataParametersX,
			...
			photonInd, countedPhotons = tm.peakFinder(WLSTraces, te.windowParametersX, te.dataParametersX,
				te.medianFactorPF, te.stdFactor, te.convWindow, te.convPower, te.convSig, te.minimaWindowPF, plotRaw=te.plotRaw, traceDir=te.traceDir, 
				showPlots=te.showPlots, savePlots=te.savePlots, c=c)		
	...
	if te.doPlots == True:
		...
		tm.pedestalPlot(pedestalSums[0::2], te.windowParametersX, te.windowParametersY1, fileName=str(te.plotsDir/'00_CsI_pedestal.png'), show=te.showPlots, save=te.savePlots)
		tm.pedestalPlot(pedestalSums[1::2], te.windowParametersX, te.windowParametersY2, fileName=str(te.plotsDir/'00_WLS_pedestal.png'), show=te.showPlots, save=te.savePlots)
	
		#Make the three types of trace histogram plots for the CsI and WLS (6 total).
		print('Plotting PHDs...')
		binsCsI = tm.plotPHD(CsISums, te.windowParametersX, te.windowParametersY1, fileName=str(te.plotsDir/'01_CsI_PHD_Full.png'), show=te.showPlots, save=te.savePlots)
		binsWLS = tm.plotPHD(WLSSums, te.windowParametersX, te.windowParametersY2, fileName=str(te.plotsDir/'01_WLS_PHD_Full.png'), show=te.showPlots, save=te.savePlots)
	
		if te.doDoubleRejection == True:
			tm.plotPHD(CsISumsCulled, te.windowParametersX, te.windowParametersY1, bins=binsCsI, fileName=str(te.plotsDir/'01_CsI_PHD_Culled.png'), show=te.showPlots, save=te.savePlots)
		...
		if te.doDoubleRejection == True:
			...
			if te.allPlots == True:	
				for i, index in enumerate(bad):
					...
					tm.plotDualTrace(CsITraces[CsITraces.columns[index]], WLSTraces[WLSTraces.columns[index]], te.windowParametersX,
